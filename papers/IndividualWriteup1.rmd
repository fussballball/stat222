...
```{r, echo = FALSE, fig.cap = "Surface Temperature at 
(178.75,-11.25) and (11.25, -66.25)"), label = "Figure 1"}
...
```{r, fig.cap = "Auto-covariance boxplots over the lag values for the timeseries at
every lat/long pair", label = "Figure 2"}
knitr::include_graphics("boxpot.jpg")
```
...
simulation start", label = "Figure 3"}
...
```{r, echo = FALSE, fig.cap = "Correlogram for Geo-spatial
data 231 months after simulation start", label = "Figure 4"}
time <- 231
...
A) for $mod^j$ in $J models:
    for $var_i$ in $I variables$:
        1) $M_{var_i} = mod^j_{.,.,.,var_i}$
        2) $T = (flatten(M_{var_i}))^T$
        3) Let $P$ such that $T^TT = P^T\Lambda_{sp} P$, then:
           $\tilde{M} = ((TP)_{.,1-N})^T$
        4) Let $Q$ such that $\tilde{M}^T\tilde{M} = Q^T\Lambda_{temp} Q$, then:
           $\hat{M} = (\tilde{M}Q)_{.,1-K}$
...
C) Perform Classical MDS:
   ...
   Let $Q_B$ such that $B = Q_B^T\Lambda_{dist}Q_B$, then:
       $\hat{\mathcal{M}} = Q_B_{.,1-2} diag{\lambda_1, \lambda_2}^{frac{1}{2}}$,
       where $\lambda_1$ and $\lambda_2$ are the two largest eigenvalues of $B$ and
       $Q_B_{.,1-2}$ are the corresponding eigenvectors.
