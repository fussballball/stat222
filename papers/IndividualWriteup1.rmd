...
A) for $mod^j$ in $J models:
    for $var_i$ in $I variables$:
        ...
        3) Let $P$ such that $T^TT = P^T\Lambda_{spatial} P$, then:
           $\tilde{M} = ((TP)_{.,1-N})^T$
        4) Let $Q$ such that $\tilde{M}^T\tilde{M} = Q^T\Lambda_{time} Q$, then:
...
C) Perform Classical MDS:
   ...
   Let $Q^B$ such that $B = Q^{BT}\Lambda_{dist}Q^B$, then:
       $\hat{\mathcal{M}} = Q^B_{.,1-2} diag{\lambda_1, \lambda_2}^{frac{1}{2}}$,
       where $\lambda_1$ and $\lambda_2$ are the two largest eigenvalues of $B$ and
       $Q^B_{.,1-2}$ are the corresponding eigenvectors.
